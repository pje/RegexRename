...
extension String {
  ...
  func matches(_ pattern: String, as: StringOrRegex = StringOrRegex.string("")) -> [NSTextCheckingResult] {
    let maybeRegex = try? NSRegularExpression(pattern: pattern)
    return(maybeRegex?.matches(in: self, range: NSRange(location: 0, length: self.count)) ?? [])
  }
